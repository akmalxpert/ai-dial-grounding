...
class InputGrounder:
    ...
    async def _update_vectorstore(self):
        """Compare and sync vectorstore ids with user ids from the user client."""
        api_users = self.user_client.get_all_users()
        users_dict = {str(user.get('id')): user for user in api_users}
        api_user_ids = set(users_dict.keys())

        vectorstore_data = self.vectorstore.get()
        vectorstore_user_ids = set(str(user_id) for user_id in vectorstore_data.get("ids", []))

        new_user_ids = api_user_ids - vectorstore_user_ids
        ids_to_delete = vectorstore_user_ids - api_user_ids
        ...
        if new_id_documents:
            if len(new_id_documents) > 50:
                batches = [new_id_documents[i:i + 50] for i in range(0, len(new_id_documents), 50)]
                await asyncio.gather(*[self.vectorstore.aadd_documents(batch) for batch in batches])
            else:
                await self.vectorstore.aadd_documents(new_id_documents)
    ...
    async def retrieve_context(self, query: str, k: int = 100, score: float = 0.2) -> str:
        ...
        relevant_docs = self.vectorstore.similarity_search_with_relevance_scores(query=query, k=k,
...
class OutputGrounder:
    ...
    async def ground_response(self, grouping_results: GroupingResults):
        print("=" * 50 + " GROUNDED RESPONSE " + "=" * 50)
        for grouping_result in grouping_results.grouping_results:
            print(grouping_result.hobby)
            print(await self._find_users(grouping_result.user_ids))
            print("=" * 50)
...
async def main():
    ...
    async with InputGrounder(embeddings, llm_client) as rag:
        ...
        while True:
            ...
            results = rag.generate_answer(augmented_prompt)
            await output_grounder.ground_response(results)
